...
public class EdgeDetection {
    ...
    /**
     * New helper method that extracts and returns the filtered boundary as a MatOfPoint.
     * This method runs the same detection steps as in drawBoundaryOnWhiteBackground but
     * returns the detected boundary (polyline) instead of drawing it.
     * @param imagePath The path to the input image.
     * @return The detected boundary as a MatOfPoint.
     */
    public static MatOfPoint getFilteredBoundary(String imagePath) {
        Mat original = Imgcodecs.imread(imagePath);
        if (original.empty()) {
            System.out.println("Error: Could not load the image!");
            return null;
        ...
        // 13. Return the final boundary as a MatOfPoint
        MatOfPoint finalContour = new MatOfPoint();
        finalContour.fromList(filteredPoints);
        return finalContour;
    }
